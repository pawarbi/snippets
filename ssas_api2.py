...
def _load_assemblies(amo_path=r"C:\Users\spawar.CREELIGHTING\Desktop\Power BI Projects\Microsoft.AnalysisServices.Tabular.DLL", adomd_path=r"C:\Users\spawar.CREELIGHTING\Desktop\Power BI Projects\Microsoft.AnalysisServices.AdomdClient.dll"):
    """
    ...
    if dt_types:
        for dtt in dt_types:
            ...
            if not df.loc[:, dtt].isna().all():
                ser = df.loc[:, dtt].map(lambda x: str(x))
